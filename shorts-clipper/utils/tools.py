...
def download_csv(args):
    ...
    cmd = "tar -xzvf %s -C %s" % (args.dataPathAVA +
                                  '/csv.tar.gz', args.dataPathAVA)
    subprocess.call(cmd, shell=True, stdout=None)
...
def download_videos(args):
    ...
    for dataType in ['trainval', 'test']:
        fileList = open('%s/%s_file_list.txt' %
                        (args.trialPathAVA, dataType)).read().splitlines()
        outFolder = '%s/%s' % (args.visualOrigPathAVA, dataType)
        for fileName in fileList:
            cmd = "wget -P %s https://s3.amazonaws.com/ava-dataset/%s/%s" % (
                outFolder, dataType, fileName)
            subprocess.call(cmd, shell=True, stdout=None)
...
def extract_audio(args):
    ...
    for dataType in ['trainval', 'test']:
        ...
        for videoPath in videos:
            audioPath = '%s/%s' % (outFolder, videoPath.split('/')
                                   [-1].split('.')[0] + '.wav')
...
def extract_audio_clips(args):
    ...
    for dataType in ['train', 'val', 'test']:
        df = pandas.read_csv(os.path.join(
            args.trialPathAVA, '%s_orig.csv' % (dataType)), engine='python')
        dfNeg = pandas.concat(
            [df[df['label_id'] == 0], df[df['label_id'] == 2]])
...
def extract_video_clips(args):
    ...
    for dataType in ['train', 'val', 'test']:
        df = pandas.read_csv(os.path.join(
            args.trialPathAVA, '%s_orig.csv' % (dataType)))
        dfNeg = pandas.concat(
            [df[df['label_id'] == 0], df[df['label_id'] == 2]])
        dfPos = df[df['label_id'] == 1]
        insNeg = dfNeg['instance_id'].unique().tolist()
        ...
        for entity in entityList:
            ...
            videoFile = glob.glob(os.path.join(
                videoDir, '{}.*'.format(videoKey)))[0]
            ...
            for _, row in insData.iterrows():
                imageFilename = os.path.join(insDir, str(
